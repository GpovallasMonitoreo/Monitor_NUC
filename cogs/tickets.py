...
class SistemaTickets(commands.Cog):
    ...
    async def cog_load(self):
        # 🟢 ESTO ES LA MAGIA: Registramos la vista global para que sobreviva a reinicios.
        self.bot.add_view(AccionesTicketGlobalView(self))
        print("✅ Vista Persistente de Tickets registrada.")
    ...
    async def reporte(self, interaction: discord.Interaction, sitio: str, foto: discord.Attachment):
        ...
        except: await interaction.followup.send("❌ Error al iniciar el reporte.", ephemeral=True)
    ...
    async def crear_ticket_final(self, interaction, datos):
        try:
            ...
            # Nota: Asegúrate de que db.crear_ticket enrute a tickets_biobox o incidencias_general según la 'Unidad_de_negocio'
            # nuevo_id = await db.crear_ticket(datos_db)
            nuevo_id = f"TICKET-TEST-{datetime.datetime.now().strftime('%H%M%S')}" # Sustituir por db real
    ...
    async def notificar_nuevo_responsable(self, ticket_id, datos, nota_extra="", hilo_url=None):
        try:
            ...
            if config_depto:
                ...
                if canal:
                    embed = discord.Embed(title=f"🔔 Asignación: {ticket_id}", description=f"[🔗 Clic aquí para ir al Ticket]({hilo_url})", color=COLOR_EMBED)
                    embed.add_field(name="Sitio", value=datos['sitio'])
                    if nota_extra: embed.add_field(name="Nota", value=nota_extra)
