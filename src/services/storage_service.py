...
class SupabaseService:
    ...
    def get_financial_overview(self):
        try:
            ...
            for f in finances:
                ...
                # CORRECCIÓN: Convertir a string y mayúsculas para evitar errores
                ctype = str(f.get('cost_type', '')).upper()
                rec = str(f.get('recurrence', '')).lower()
                
                # Compatibilidad o fallback
                if not ctype or ctype == 'NONE':
                    old_type = str(f.get('type', '')).upper()
    ...
    def get_device_detail(self, device_id):
        try:
            ...
            for r in records:
                ...
                # CORRECCIÓN: Normalizar a mayúsculas
                ctype = str(r.get('cost_type', '')).upper()
                rec = str(r.get('recurrence', '')).lower()
                ...
                # Manejo de columnas específicas del INSERT anterior (cost_pantalla, etc)
                # Si 'amount' viene vacío pero tienes las columnas individuales, el trigger
                # ya debió haber llenado 'total_capex' en la DB.
                # Si prefieres usar el cálculo del trigger en vez de sumar aquí:
                # if r.get('total_capex'): data['totals']['capex'] = r.get('total_capex')
    ...
    def save_cost_entry(self, payload):
        try:
            # Asegurar que guardamos en mayúsculas para consistencia
            ctype = payload['cost_type'].upper()
            ...
            record = {
                ...
                "type": "sale" if ctype == 'REVENUE' else "expense"
            }
